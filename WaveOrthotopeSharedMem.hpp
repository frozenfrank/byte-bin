#ifndef WAVE_ORTHOTOPE_SHARED_MEM_H
#define WAVE_ORTHOTOPE_SHARED_MEM_H
#include <cstdlib>
#include <format>
...
class WaveOrthotopeSharedMem {

    const std::string inputFile;
    const std::string outputFile;

public:
    WaveOrthotopeSharedMem(const auto input, const auto output): inputFile(input), outputFile(output) {}
    ...
    bool solve(){
        ...
        WaveOrthotope wave = WaveOrthotope(i, j, c);
        for (int k = 0; k < i; ++k) {
            for (int l = 0; l < j; ++l) {
    ...
    bool write(unsigned long int n, unsigned long int i, unsigned long int j, double c, double t, WaveOrthotope wave, std::string out){
