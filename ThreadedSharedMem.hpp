#ifndef THREADED_SHARED_MEM_H
#define THREADED_SHARED_MEM_H
#include <cstdlib>
#include <format>
#include <fstream>      // std::ifstream
#include <math.h>
#include <string>
#include "ThreadedWave.hpp"
#include "binary_io.hpp"


class ThreadedSharedMem {

    const std::string inputFile;
    const std::string outputFile;

public:
    ThreadedSharedMem(const auto input, const auto output): inputFile(input), outputFile(output) {}
    ...
    bool solve(){
        ...
        ThreadedWave wave = ThreadedWave(i, j, c);
        for (int k = 0; k < i; ++k) {
            for (int l = 0; l < j; ++l) {
        ...
        while (wave.energy() > (i-2)*(j-2)*0.001) {
            ...
            if (interval > 0 && (fmod(t+0.002, interval)< 0.004)){
                ...
                wave.write(n, i, j, c, t, outputFile);
            }
        }
    ...
    // bool write(unsigned long int n, unsigned long int i, unsigned long int j, double c, double t, ThreadedWave wave, std::string out){
